...
export default function MedManagerScreen() {
  ...
  const [meds, setMeds] = useState<any[]>([]);  // ← thêm type
  const medFormRef = useRef<BottomSheet>(null);

  useEffect(() => {
    load("meds").then((data) => {
      if (Array.isArray(data)) setMeds(data);  // ← check an toàn
    });
  }, []);

  const addMed = async (med: any) => {  // ← thêm type
    const updated = [...meds, med];
    setMeds(updated);
    await save("meds", updated);
  };
  ...
  return (
    <SafeAreaView style={s.safe} edges={["top"]}>
      ...
      <MedForm ref={medFormRef} onSave={addMed} />
    </SafeAreaView>
  );
}
